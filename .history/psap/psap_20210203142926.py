...
def export_matrix(name, fasta_path, out_path, operating_system='Windows', set_labels=True, identifier_name=""):
    # Change pathing
    ...
    if (set_labels):
        ...
        for prot_id in uniprot_ids:
            ...
            pkl_labeled = Path(out_path,name+'_llps_f2f_'+date+'_labeled_'+'.pkl')    
            data.df.to_pickle(pkl_labeled)
            return(data.df)
    else:
        pkl_unlabeled = Path(out_path,name+'_llps_f2f_'+date+'.pkl')
        data.df.to_pickle(pkl_unlabeled)
        print(pkl_unlabeled)
        return data.df
