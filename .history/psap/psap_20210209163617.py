...
class MakeMatrix:
    def __init__(self, self.dbdasta):
        self.df = pd.DataFrame()
        self.db_fasta = dbdasta
        self.add_features()

    def add_features(self):
        executables = [
             'self.fasta2df(self.dbdasta)',
    ...
    def fasta2df(self, self.dbdasta):
        rows = list()
        with open(self.dbdasta) as f:
            for record in SeqIO.parse(self.dbdasta, 'fasta'):
                seqdict = dict()
                seq = str(record.seq)
